...
Сравним производительность сгенерированного x64 кода с производительностью [написанного ранее](https://github.com/foxidokun/ReverseLang) компилятора для эмулятора стекового процессора.
В качестве тестовых задач возьмем две программы: решение квадратного уравнения $2x² + 2x - 12 = 0$ и расчет 15го числа Фибоначчи, код которых расположен в `examples/`. 

**Методика измерений**

В каждой программе соответствующий алгоритм запускается 10000 раз, то есть квадратное уравнение решается 10000 раз за запуск. При этом проводится
пять запусков, результаты которых усредняются и считается погрешность.
